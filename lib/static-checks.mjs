...
// Global roots that can be used to reach non-deterministic / privileged APIs.
const GLOBAL_ROOTS = new Set(['globalThis', 'window', 'self', 'global']);

// What is actually dangerous OFF a global root. This used to block EVERY member access on one, which
// was wrong in a way that only showed up in production: a browser game legitimately reaches for
// `globalThis.AudioContext` and `globalThis.addEventListener`, and blanket-blocking them made the
// platform's own published game (garuda — AudioContext, webkitAudioContext, add/removeEventListener,
// and nothing else) fail its own conformance scan. The rule landed AFTER those versions were
// approved, so it rejected them retroactively and the studio's Verify started failing on saves that
// had verified fine days earlier.
//
// The scan is defense-in-depth, NOT the boundary — the sealed isolate is. Blocking `AudioContext`
// buys nothing there (the isolate has no such global) and costs a real, conforming game. So: block
// the escapes by name, allow the rest, and keep COMPUTED access blocked because `globalThis['fe'+
// 'tch']` is exactly the obfuscation this scan exists to make inconvenient.
const GLOBAL_ESCAPE_PROPS = new Set([
  'eval', 'Function', 'constructor',                                   // code injection
  'fetch', 'XMLHttpRequest', 'WebSocket', 'EventSource', 'sendBeacon', // network
  'navigator', 'crypto',                    // fingerprinting + crypto.getRandomValues (non-determinism)
  'import', 'importScripts', 'require', 'process', 'module',           // module / system
  'document',                                                          // window.document.cookie
  'open', 'postMessage', 'parent', 'top', 'opener', 'frames',          // window / frame escape
  'indexedDB', 'caches',                                               // persistence
]);

// Returns a violation string for a member access on a global root, or null when it is benign.
function globalRootEscape(n, objName, propName) {
  const on = objName(n);
  if (!GLOBAL_ROOTS.has(on)) return null;
  if (n.computed && n.property.type !== 'Literal') return `${on}[…] (computed global access)`;
  const p = propName(n);
  return p !== null && GLOBAL_ESCAPE_PROPS.has(p) ? `${on}.${p} (global escape)` : null;
}

// Walk the AST collecting hard (block) / soft (warn) forbidden-API hits. This is defense-in-depth,
// NOT the security boundary — the isolate is. It closes the cheap, un-obfuscated bypasses; it can't
// catch e.g. `const p = process` aliasing, which the capability-free isolate handles at runtime.
function scan(ast) {
  const hard = [], soft = [];
  ...
  const objName = (n) => (n.type === 'MemberExpression' && n.object.type === 'Identifier') ? n.object.name : null;
  const propName = (n) => n.type !== 'MemberExpression' ? null
    : (n.property.type === 'Identifier' && !n.computed) ? n.property.name
  ...
  walk.simple(ast, {
    CallExpression(n) {
      ...
      if (c.type === 'Identifier') {
        ...
        if (c.name === 'Function') hard.push(`L${L(n)}: Function() (code injection)`);
      }
      if (c.type === 'MemberExpression') {
        if (propName(c) === 'constructor') hard.push(`L${L(n)}: .constructor(...) (Function via constructor.constructor)`);
        // No global-root branch here: walk.simple also visits this callee as a MemberExpression,
        // where globalRootEscape() judges it precisely. A blanket rule here flagged every
        // `globalThis.addEventListener(...)` as `globalThis[...]()`.
      }
      // indirect eval / Function: (0, eval)('…')
      if (c.type === 'SequenceExpression') {
        const last = c.expressions[c.expressions.length - 1];
        if (last?.type === 'Identifier' && (last.name === 'eval' || last.name === 'Function'))
          hard.push(`L${L(n)}: indirect ${last.name} (code injection)`);
      }
    ...
    MetaProperty(n) { hard.push(`L${L(n)}: import.meta (module/filesystem introspection)`); },
    MemberExpression(n) {
      if (memberIs(n, 'document', 'cookie')) hard.push(`L${L(n)}: document.cookie`);
      const on = objName(n);
      if (on === 'process') hard.push(`L${L(n)}: process.${propName(n) ?? '*'} (system access)`);
      const esc = globalRootEscape(n, objName, propName);
      if (esc) hard.push(`L${L(n)}: ${esc}`);
      const gp = GLOBAL_ROOTS.has(on) ? propName(n) : null;
      if (on === 'localStorage' || on === 'sessionStorage' || gp === 'localStorage' || gp === 'sessionStorage')
        soft.push(`L${L(n)}: ${gp || on} — settings/UI only`);
    },
    // A conforming game is ONE self-contained file — zero imports (relative OR bare). Block every
    // import/re-export-with-source; `import 'child_process'`/`'fs'`/`'net'` was the biggest hole.
    ImportDeclaration(n) { hard.push(`L${L(n)}: import "${n.source?.value}" (games are self-contained — no imports)`); },
    ExportNamedDeclaration(n) { if (n.source) hard.push(`L${L(n)}: re-export from "${n.source.value}" (no imports)`); },
    ExportAllDeclaration(n) { hard.push(`L${L(n)}: re-export from "${n.source?.value}" (no imports)`); },
  });
  return { hard, soft };
}
