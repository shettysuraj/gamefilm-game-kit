...
// Global roots whose members/calls are an escape hatch to non-deterministic / privileged APIs.
// A conforming, self-contained game never touches these (render gets ctx + W/H; sim is pure).
  ...
  walk.simple(ast, {
    CallExpression(n) {
      ...
      if (c.type === 'MemberExpression') {
        ...
        if (GLOBAL_ROOTS.has(objName(c))) hard.push(`L${L(n)}: ${objName(c)}[...]() (global escape)`);
      }
    ...
    MemberExpression(n) {
      ...
      if (GLOBAL_ROOTS.has(on)) hard.push(`L${L(n)}: ${on}.${propName(n) ?? '*'} (global escape)`);
      if (on === 'localStorage' || on === 'sessionStorage') soft.push(`L${L(n)}: ${on} — settings/UI only`);
    },
