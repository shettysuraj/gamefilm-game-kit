...
// ---- 1. static source scan (two tiers) ----
// HARD = security / portability; no legitimate use in a self-contained game, and NOT covered
//        by the determinism check. These block.
// WARN = determinism-relevant; legitimate in render/UI, and the determinism double-run is the
//        authoritative arbiter. These flag for review but don't block.
...
const HARD = [
  ...
  [/\bnew\s+Function\b/, 'new Function (code injection)'],
  [/(^|[^.\w])import\s*\(/, 'dynamic import() (loads external code)'],
  [/\bdocument\.cookie\b/, 'document.cookie'],
];
const SOFT = [
  [/\bMath\.random\b/, 'Math.random — render/UI only, must NOT affect the sim'],
  [/\bDate\.now\b/, 'Date.now — render/UI only'],
  [/\bperformance\.now\b/, 'performance.now — render/UI only'],
  [/\bnew\s+Date\b/, 'new Date — render/UI only'],
  [/\blocalStorage\b/, 'localStorage — settings/UI only, never the sim'],
  [/\bsessionStorage\b/, 'sessionStorage — settings/UI only'],
];
const RELATIVE_IMPORT = /(?:\bfrom\s*|\bimport\s*|\brequire\s*\(\s*)['"]\.\.?\//;
{
  const lines = src.split('\n');
  const hard = [], soft = [];
  lines.forEach((ln, i) => {
    // Skip full-line comments — avoids false positives like a comment that mentions
    // `import './…'` (garuda). Inline-trailing comments + strings still need the AST scan
    // (Phase 1.2 piece #3); this is the cheap pre-AST guard.
    const t = ln.trim();
    if (t.startsWith('//') || t.startsWith('*') || t.startsWith('/*')) return;
    for (const [re, label] of HARD) if (re.test(ln)) hard.push(`L${i + 1}: ${label}`);
    for (const [re, label] of SOFT) if (re.test(ln)) soft.push(`L${i + 1}: ${label}`);
    if (RELATIVE_IMPORT.test(ln)) hard.push(`L${i + 1}: relative import (game.js must be self-contained for archiving)`);
  });
  check('static (hard): no network / eval / dynamic-import / relative-import / cookies', hard.length === 0, hard.slice(0, 12).join('; '));
  warn('static (warn): determinism-sensitive APIs are render/UI-only', soft.length === 0,
    soft.length ? `${soft.length} hit(s) — OK only because the determinism check passes; verify none feed the sim. ${soft.slice(0, 6).join('; ')}${soft.length > 6 ? ' …' : ''}` : '');
}
